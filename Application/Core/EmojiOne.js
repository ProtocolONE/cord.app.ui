...
ns.htmlToShortname = function(str) {
    if (!ns.mappedUnicode) {
        ns.mappedUnicode = ns.mapShortToUnicode();
    }

    str = str.replace(/<img\s+src\s*=\s*"(.+?)"(["'][^"']+["']|[^>]+)>/gi, function(attr, src) {

        if( (src == undefined) || (src == '')/* || (!(ns.unescapeHTML(src) in ns.asciiList))(*/ ) {
            // if the shortname doesnt exist just return the entire match
            return attr;
        }
        else {
            src = ns.unescapeHTML(src);
            var shortname = src.toUpperCase().replace(ns.imagePathPNG.toUpperCase(), "").replace(".PNG", "");

            if (ns.mappedUnicode.hasOwnProperty(shortname)) {
                return ns.mappedUnicode[shortname];
            }
        ...
        if( (shortname == undefined) || (shortname == '') || (!(shortname in ns.emojioneList)) ) {
            ...
            if(ns.imageType == 'png') {
                if(ns.sprites) {
                    ...
                    replaceWith = '<img ' + ns.addedImageProps + ' class="emojione" alt="'+alt+'" src="'+ns.imagePathPNG+unicode+'.png'+ns.cacheBustParam+'"/>';
                }
            }
            else {
                // svg
            ...
            if( (m3 == undefined) || (m3 == '') || (!(ns.unescapeHTML(m3) in ns.asciiList)) ) {
                ...
                if(ns.imageType == 'png') {
                    if(ns.sprites) {
                        ...
                        replaceWith = m2 + '<img ' + ns.addedImageProps + ' class="emojione" alt="'+alt+'" src="'+ns.imagePathPNG+unicode+'.png'+ns.cacheBustParam+'"/>';
                    }
                }
                else {
                    // svg
